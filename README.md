...
Аргументы передаются одним объектом JSON, `-` вместо него читает их из stdin. `--list` печатает имена всех инструментов. В списочном вызове `"all": true` пролистывает все страницы, а `"fields": ["id", "title"]` оставляет в строках только названные поля.
