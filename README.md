...
Если у выбранной компании уже есть действующий ключ, `setup` берёт его, а новый создаёт, только когда действующих нет: на аккаунт положено не больше 30 ключей. Чтобы перевыпустить ключ, удалите старый через `auth_delete_key` и запустите `setup` снова.
...
Аргументы передаются одним объектом JSON, `-` вместо него читает их из stdin. `--list` печатает все инструменты с методом, путём, фильтрами и обязательными полями. В списочном вызове `"all": true` пролистывает все страницы, а `"fields": ["id", "title"]` оставляет в строках только названные поля.
