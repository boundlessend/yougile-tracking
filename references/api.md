...
```bash
$YG tasks_list '{"columnId":"<id>","limit":1000}'
$YG tasks_list '{"columnId":"<id>","limit":1000,"offset":1000}'
```
...
- Успех это `200`, при создании `201`. Ошибка возвращает код `3xx`, `4xx` или
  ...
  повторяются трижды, после чего ошибка поднимается.
- Ключ работает с правами аккаунта, который его создал. Чего аккаунт не может в
  интерфейсе, того он не может и через API.
...
`python3 ~/.claude/skills/yougile/scripts/yg.py <инструмент> '<json>'`. Вместо
